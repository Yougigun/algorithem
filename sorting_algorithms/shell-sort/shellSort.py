...
def shellsort(A):
    ...
    while gap > 0:
        i = gap
        while i < n:
            # Ready to do insertion sort for first group first element
            temp = A[i]
            j = i - gap
            # Put the bigger value one gap forward
            while j >= 0 and A[j] > temp:
                A[j+gap] = A[j]
                j = j - gap
            # Put the current value in the correct index
            A[j+gap] = temp
            # for next group first element
            i = i + 1
        gap = gap // 2
